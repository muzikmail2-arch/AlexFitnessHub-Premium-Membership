...
export default function QuickTipAdvocate({ activityLogs, goal }: QuickTipProps) {
  ...
  const fetchQuickTip = async () => {
    ...
    try {
      if (isMockFirebase) {
        // Mock tip generation
        const mockTips = [
          "### BIOMECHANICAL ECCENTRICS\n1. **3-Second Negative**: Focus on a controlled eccentric phase to maximize myofibrillar micro-tears.\n- Keep perfect posture during movement.\n- Hydrate with lemon-cucumber water.",
          "### HYDRATION PROTOCOL\n1. **Pre-Workout Intake**: Consume 500ml of ambient water 30 minutes before your workout to increase cellular volume.",
          "### MACRO CALIBRATION\n1. **Protein Target**: Prioritize 1.6g to 2.2g of protein per kg of bodyweight for muscle recovery."
        ];
      ...
      setError(`AI Coach Error: ${err.message || "Failed to retrieve active recommendations."}`);
    } finally {
      setLoading(false);
    }
  };

  const activityLogsLength = activityLogs.length;
  const latestLogId = activityLogs[0]?.id || "";
  ...
  }, [activityLogsLength, latestLogId, goal]);
